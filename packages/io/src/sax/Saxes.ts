...
export class SaxesParser<O extends SaxesOptions = {}> {
  ...
  fileName?: string;
  nameStartCheck: (c: number) => boolean;
  nameCheck: (c: number) => boolean;
  ...
  entity!: string;
  q!: null | number;
  tags!: SaxesTagIncomplete[];
  tag!: SaxesTagIncomplete | null;
  topNS!: Record<string, string> | null;
  chunk!: string;
  ...
  //
  prevI: number=0;
  carriedFromPrevious?: string;
  forbiddenState!: number;
  attribList!: (SaxesAttributeNSIncomplete | SaxesAttributePlain)[];
  state!: number;
  reportedTextBeforeRoot!: boolean;
  reportedTextAfterRoot!: boolean;
  closedRoot!: boolean;
  sawRoot!: boolean;
  xmlDeclPossible!: boolean;
  xmlDeclExpects!: string[];
  entityReturnState?: number;
  processAttribs!: (this: this) => void;
  positionAtNewLine!: number;
  doctype!: boolean;
  getCode!: () => number;
  isChar!: (c: number) => boolean;
  pushAttrib!: (name: string, value: string) => void;
  _closed!: boolean;
  ...
  openTagStartHandler?: OpenTagStartHandler<O>;
  openTagHandler?: OpenTagHandler<O>;
  closeTagHandler?: CloseTagHandler<O>;
  cdataHandler?: CDataHandler;
  errorHandler?: ErrorHandler;
  endHandler?: EndHandler;
  readyHandler?: ReadyHandler;
  attributeHandler?: AttributeHandler<O>;
  ...
  getCode10(): number {
    ...
    // performance.mark('getCode-start');
    return final;
  }
   ...
   *
   * @returns The character code that made the capture end, or ``EOC`` if we hit
   * the end of the chunk. The return value cannot be NL_LIKE: NL is returned
   * instead.
   */
  captureTo(chars: number[]): number {
    let { i: start } = this;
    const { chunk } = this;
    while (true) {
      const c = this.getCode();
   ...
   *
   * @returns ``true`` if we ran into the character. Otherwise, we ran into the
   * end of the current chunk.
   */
  captureToChar(char: number): boolean {
    let { i: start } = this;
    const { chunk } = this;
    while (true) {
      let c = this.getCode();
   ...
   * this parser.
   *
   * @returns The character code that made the test fail, or ``EOC`` if we hit
   * the end of the chunk. The return value cannot be NL_LIKE: NL is returned
   * instead.
   */
  captureNameChars(): number {
   ...
   * Skip white spaces.
   *
   * @returns The character that ended the skip, or ``EOC`` if we hit
   * the end of the chunk. The return value cannot be NL_LIKE: NL is returned
   * instead.
   */
  skipSpaces(): number {
  ...
  sDTDCommentEnding(): void {
    const c = this.getCodeNorm();
    this.text += String.fromCodePoint(c);
    this.state = c === MINUS ? S_DTD_COMMENT_ENDED : S_DTD_COMMENT;
  }

  sDTDCommentEnded(): void {
    const c = this.getCodeNorm();
    this.text += String.fromCodePoint(c);
    if (c === GREATER) {
      this.state = S_DTD;
...
/^[A-Za-z0-9][A-Za-z0-9._-]*$/.");
  ...
  pushAttribPlain(name: string, value: string): void {
   ...
   */
  parseEntity(entity: string): string {
