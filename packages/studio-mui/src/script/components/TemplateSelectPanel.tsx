...
const TemplateIconButtonProps = { fontSize: '1rem' };
...
const TemplateSelectPanel = memo(forwardRef<HTMLDivElement, TemplateSelectPanelProps>(
  ...
  const icon = (file: DirectoryFile) => {
    ...
    if (metaIcon === 'macro') {
      return <ScriptMacroIcon  sx={TemplateIconButtonProps}/>;
    }
    if (metaIcon === 'formula') {
      return <ScriptFormulaIcon sx={TemplateIconButtonProps}/>;
    }
    if (metaIcon === 'autostart') {
      return <WebhookOutlined sx={TemplateIconButtonProps} />;
    }
    return null;
  }
