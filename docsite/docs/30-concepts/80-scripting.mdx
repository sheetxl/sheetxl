...
* **Vectorized  API**: Perform data-science-style range operations with full spreadsheet context — presentation and interaction included.
...
:::note
While we use the term User Defined Functions to align with the industry standard, **All SheetXL built-in functions** are defined using the same API and are made freely available at [functions](https://www.github.com/sheetxl/functions).
:::
...
| **Persistence State** | Hidden workbook storage   | <span title="Not available in Office Scripts">❌</span> Not supported            | 'useState' serialization <span title="Feature under development">🚧</span>        |
...
| **Formula Functions** | UDFs via `Function`       | <span title="Not available in Office Scripts">❌</span> Not supported            | Exported value-returning functions |
| **Global State**      | Module variables          | <span title="No persistent global variables in Office Scripts">❌</span> No true globals           | `globalThis` for shared state     |
| **Extensibility**     | COM / Add-ins (limited)   | <span title="No module imports in Office Scripts">❌</span> No imports                | ES module imports                 |
